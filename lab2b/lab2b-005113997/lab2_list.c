...
SortedList_t *heads; // head of list to be sorted
...
int main(int argc, char * argv[]) {
    ...
    heads = (SortedList_t*)malloc(lists_num * sizeof(SortedList_t));
    mutexes = (pthread_mutex_t*)malloc(lists_num * sizeof(pthread_mutex_t));
    spin_locks = (int*)malloc(lists_num * sizeof(int));
    int l;
    for(l = 0; l < lists_num; l++)
    {
        pthread_mutex_init(&mutexes[l], NULL);
        spin_locks[l] = 0;
        heads[l].next = &heads[l];
        heads[l].prev = &heads[l];
        heads[l].key = NULL;
    }
    ...
    int list_len = 0;
    int m;
    for(m = 0; m < lists_num; m++)
    {
        list_len += SortedList_length(&heads[m]);
    }
    if(list_len != 0)
    {
    ...
    if(sync_option == 'x')
    {
        ...
        {
            fprintf(stdout, "list-none-none,%d,%d,%d,%d,%lld,%lld,%d\n", threads_num, iterations_num, lists_num, total_operations, total_time, average_operation_time, 0);
        }
    ...
    else if(sync_option == 'm')
    {
        if(strcmp(yield_option, "x") == 0) // no yield
        {
            fprintf(stdout, "list-none-m,%d,%d,%d,%d,%lld,%lld,%lld\n", threads_num, iterations_num, lists_num, total_operations, total_time, average_operation_time, average_wait_for_lock_time);
        }
        else if(strcmp(yield_option, "i") == 0)
        {
...
void* list_thread_func(void* arg) {
    ...
    if(sync_option == 'x')
    {
        ...
        {
            int hash = hash_function(list[i].key);
            SortedList_insert(&heads[hash], &list[i]);
            i += threads_num;
        }
        int list_len = 0;
        int k;
        for(k = 0; k < lists_num; k++) // get total list len by iterating through every sublist
        {
            list_len += SortedList_length(&heads[k]);
        }
        if(list_len == -1)
        {
        ...
        {
            int hash = hash_function(list[i].key);
            SortedListElement_t *lookup = SortedList_lookup(&heads[hash], list[i].key);
            if(lookup == NULL)
            {
    ...
    else if(sync_option == 'm')
    {
        ...
        for(i = this_thread_info->index_of_thread; i < elements_num; )
        {
            ...
            int lock_error = pthread_mutex_lock(&mutexes[hash]);
            ...
            int unlock_error = pthread_mutex_unlock(&mutexes[hash]);
            if(unlock_error != 0)
            {
        ...
        int j;
        for(j = 0; j < lists_num; j++) // lock all mutexes to get total list len
        {
            int lock_error = pthread_mutex_lock(&mutexes[j]);
            if(lock_error != 0)
            {
                fprintf(stderr, "Error on pthread_mutex_lock system call.\n");
                exit(1);
        ...
        for(j = 0; j < lists_num; j++) // unlock all mutexes
        {
            int unlock_error = pthread_mutex_unlock(&mutexes[j]);
            if(unlock_error != 0)
            {
                fprintf(stderr, "Error on pthread_mutex_unlock system call.\n");
                exit(1);
    ...
    else if(sync_option == 's')
    {
        ...
        int clock_error = clock_gettime(CLOCK_MONOTONIC, &start_time_thread);
        int j;
        for(j = 0; j < lists_num; j++) // lock all spin locks to get total list len
        {
        ...
        for(j = 0; j < lists_num; j++) // unlock all spin locks
        {
...
int hash_function(const char *str) // hash function djb2 by Dan Bernstein
{
    int hash = 5381;
    int c;
    while((c = *str++))
