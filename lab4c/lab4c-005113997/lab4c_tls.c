...
void button_pressed() // shutdown
{
    ...
    int error = SSL_write(ssl_main, output, strlen(output));
    if(error <= 0)
    {
        fprintf(stderr, "Error on SSL_write call.\n");
        exit(2);
    }
...
int main(int argc, char * argv[]) {
    ...
    if(ssl_context == NULL)
    {
        fprintf(stderr, "Error on SSL_CTX call.\n");
        exit(2);
    }
    ssl_main = SSL_new(ssl_context);
    if(ssl_main == NULL)
    {
        fprintf(stderr, "Error on SSL_new call.\n");
        exit(2);
    }
    int ssl_fd = SSL_set_fd(ssl_main, socket_fd);
    if(ssl_fd == 0)
    {
        fprintf(stderr, "Error on SSL_set_fd call.\n");
        exit(2);
    }
    int ssl_connect = SSL_connect(ssl_main);
    if(ssl_connect <= 0)
    {
        fprintf(stderr, "Error on SSL_connect call.\n");
        exit(2);
    }
    ...
    while(run_flag == 1)
    {
        ...
        size_t read_size_stdin = SSL_read(ssl_main, &buffer_stdin, 512);
